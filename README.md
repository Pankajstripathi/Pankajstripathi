# ⭐ Pankaj tripathi — Software Developer

<p align="center">
 <img src="https://readme-typing-svg.demolab.com?font=Fira+Code&pause=900&color=0E75B6&center=true&vCenter=true&width=750&lines=%20%20Software+Developer+%7C+Full-Stack+Engineer+%7C+AI+%26+LLM+Integrations;Next.js+15+%7C+FastAPI+%7C+Prisma+%7C+Gemini+1.5+Flash;Building+AI+products+that+solve+real+problems" />
</p>

<p align="center">
  <img src="https://komarev.com/ghpvc/?username=Pankajstripathi&color=0e75b6" />
</p>

---

## 👋 Hi, I'm Pankaj

I'm a **software developer** with a background in **BSc IT**, passionate about building real-world products using **Next.js, FastAPI, Prisma, AI/LLM systems, Docker**, and clean development practices.

I enjoy transforming ideas into **scalable, intelligent, user-friendly systems**, especially in the legal-tech, communication, and AI-driven automation space.

---

# 🚀 What I’m Working On

## 🟦 OutRiskAI – Legal Document Intelligence Platform

AI-powered platform that analyzes and summarizes PDFs, contracts, Aadhaar, PAN, loan documents, and more.

- Gemini 1.5 Flash for document parsing  
- Contract summary + risk analysis  
- ID document extraction (Aadhaar, PAN, Property Docs)  
- Chat-with-contract (streaming)  
- Marketplace for lawyers  
- Smart PDF text & image pipeline  
- Prisma + PgVector semantic search  
- Notifications via Socket.IO / Pusher  
- NextAuth (Email, Credentials, Google, Guest)

🔗 **Private repo**
https://github.com/ssingularitytech/Outriskai
---

## 🟩 OUTCaller – Smart Outbound Calling System

- Next.js + FastAPI + Twilio Voice  
- Real-time call logs, events, metrics  
- Templates, organizations, roles  
- Invite + member management  
- Complete communication dashboard

🔗https://github.com/ssingularitytech/OutCaller

---

## 🟪 VectorAI – Vector Search Engine

- PgVector + Prisma  
- Fast document embedding search  
- Used inside OutRiskAI for contract lookup  
- FastAPI backend with modular architecture


---

## 🟧 PG Loan / Property Verification System

- Tenant verification  
- Document extraction + risk scoring  
- Aadhaar/PAN metadata extraction  
- Loan agreement automation

---

# 🧠 About Me

- 🎓 **BSc IT Graduate**  
- 💼 Full-stack developer (frontend + backend + AI)  
- ☕ Huge coffee person  
- 📚 Learning: **Advanced LLM Engineering, AWS, System Design**  
- ✍️ Writing about debugging, AI pipelines, contract parsing, production apps  

---

# 🛠️ Tech Stack & Tools

## Languages
![JS](https://img.shields.io/badge/JavaScript-F7DF1E?logo=javascript&logoColor=black)
![Python](https://img.shields.io/badge/Python-3776AB?logo=python&logoColor=white)

## Frameworks
![Next.js](https://img.shields.io/badge/Next.js-000000?logo=nextdotjs&logoColor=white)
![FastAPI](https://img.shields.io/badge/FastAPI-009688?logo=fastapi&logoColor=white)
![React](https://img.shields.io/badge/React-20232A?logo=react&logoColor=61DAFB)

## AI / LLM
![Gemini](https://img.shields.io/badge/Gemini%201.5%20Flash-4285F4?logo=google&logoColor=white)
![OpenAI](https://img.shields.io/badge/OpenAI-412991?logo=openai&logoColor=white)

## Databases
![PostgreSQL](https://img.shields.io/badge/PostgreSQL-316192?logo=postgresql&logoColor=white)
![PgVector](https://img.shields.io/badge/pgvector-316192?logo=postgresql&logoColor=white)
![Prisma](https://img.shields.io/badge/Prisma-2D3748?logo=prisma&logoColor=white)

## Tools
![Docker](https://img.shields.io/badge/Docker-2496ED?logo=docker&logoColor=white)
![Redis](https://img.shields.io/badge/Redis-DC382D?logo=redis&logoColor=white)
![SendGrid](https://img.shields.io/badge/SendGrid-00B5E2?logo=sendgrid&logoColor=white)
![GitHub Actions](https://img.shields.io/badge/GitHub%20Actions-2088FF?logo=githubactions&logoColor=white)

---

# 📦 Pinned Projects

<p align="center">
  <a href="https://github.com/ssingularitytech/OutCaller">
    <img src="https://github-readme-stats.vercel.app/api/pin/?username=Pankajstripathi&repo=OUCaller&theme=radical" />
  </a>
  <a href="https://github.com/ssingularitytech/OutriskAI">
    <img src="https://github-readme-stats.vercel.app/api/pin/?username=Pankajstripathi&repo=VectorAI&theme=radical" />
  </a>
</p>

---

# 📈 GitHub Stats

<!-- <p align="center">
  <img width="48%" src="https://github-readme-stats.vercel.app/api?username=Pankajstripathi&show_icons=true&theme=radical" />
  <img width="48%" src="https://github-readme-stats.vercel.app/api/top-langs/?username=Pankajstripathi&layout=compact&theme=radical" />
</p>

<p align="center">
  <img src="https://github-readme-streak-stats.herokuapp.com/?user=Pankajstripathi&theme=tokyonight" />
</p> -->

---

# 📫 Connect With Me

- 📧 Email: *Pankajtripathi0881@gmail.com*  

---

# ⭐ Support My Work

- ⭐ Star my repositories  
- 🛠️ Share feedback  
- 🤝 Collaborate on AI/full-stack projects  

---

# 🙏 Thanks for visiting!

Always building something new — feel free to reach out anytime!
